...
class Genero
{
    ...
    public function consultarGenero($nombre){
        $registros = mysqli_query($this->conexionBD(),"SELECT * FROM generos where nombre = '$nombre'")or die("Problemas en la busqueda".mysqli_error($this->conexionBD()));
        echo  '<h1>Generos</h1>
        <form action="adminGenerosC.php" method="posty" class="consultar">
            <input type="search" name="consultar" placeholder="Busca una categoria">
            <input type="submit" value="Buscar">
            <input type="hidden" name="opcion" value="5">
        </form>';
        if($reg = mysqli_fetch_array($registros)){
            echo '<table><tr><th>Codigo</th><th>Genero</th><th colspan="2">Acciones</th></tr><tr>';
            echo "<td>" . $reg[0] . "</td>";
            echo "<td>" . $reg[1] . "</td>";
            echo '<td><form action="adminGenerosC.php" method="post">
            <input type="hidden" name="opcion" value="3">
            <input type="hidden" name="id_genero" value="'.$reg[0] .'">
            <input type="submit" value="Modificar" id="verde">
             </form></td></table>';
        }else{
            echo '<div class="falloPublicar">Este genero no existe<button><a href="../vistas/admin/adminGeneros.php">Aceptar</a></button></div>';
        }
    }
